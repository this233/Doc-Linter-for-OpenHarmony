...
>由于输入法窗口的特殊性，根据窗口策略，部分接口创建窗口时会由于输入法应用无系统签名而失败。比如bindContextMenu（默认子窗实现）、指定showInSubwindow为true的openCustomDialog等弹窗接口，此时建议开发者使用非子窗的接口，如bindMenu，或者指定showInSubwindow为false。
