...
console.info('bluetooth l2capPsm Number: ' + l2capPsm);
```
## socket.sppAccept
...
| 名称     | 类型                | 只读   | 可选   | 说明          |
| ------ | ------------------- | ---- | ---- | ----------- |
| uuid   | string              | 否    | 否    | spp单据的uuid。L2cap链路中该参数非必须，须指定为空字符串''。 |
| secure | boolean             | 否    | 否    | 是否是安全通道。true表示是安全通道，false表示非安全通道。    |
| type   | [SppType](#spptype)            | 否    | 否    | Spp链路类型。    |
| psm   | number              | 否    | 是    | 用于L2cap链路中连接的建立与管理，默认值为-1。在BR设备中psm值可以主动选择或者自动分配，其范围为0x00-0xffff,同时需要满足psm&0x0101 == 0x0001，在BLE设备中，psm值必须自动分配，需要通过[getL2capPsm](#socketgetl2cappsm20)获取,有效值范围为1-255。|
...
| SPP_L2CAP | 1    | 表示针对BR设备的l2cap链路类型。 |
| SPP_L2CAP_BLE | 2    | 表示针对BLE设备的l2cap链路类型。 |